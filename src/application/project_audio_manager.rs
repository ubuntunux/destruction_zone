...
use rodio::{self, source::Source, StreamError};
use cpal;
...
impl ProjectAudioManager {
    pub fn create_audio_manager() -> Box<ProjectAudioManager> {
        let device = std::thread::spawn(|| cpal::default_host()
            .default_output_device()
            .ok_or(StreamError::NoDevice).expect("")).join().unwrap();

        // let device = std::thread::spawn(|| cpal::default_host().default_output_device().unrwap())
        //     .join()
        //     .unwrap();
