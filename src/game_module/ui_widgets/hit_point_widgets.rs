...
pub struct ShieldPointWidget {
    pub _shield_point_layer: *mut WidgetDefault,
    pub _shield_point_bar: *mut WidgetDefault,
}
...
impl HullPointWidget {
    pub fn create_hull_point_widget(parent_widget: &mut dyn Widget) -> HullPointWidget {
        ...
        let hull_point_bar = create_hit_point_bar_widget(unsafe { &mut *hull_point_layer }, get_color32(255, 75, 0, 75));
        HullPointWidget {
    ...
    pub fn update_hull_point_widget(&self, hull_point: f32, max_hull_point: f32) {
        ...
        let hull_point_bar = unsafe { self._hull_point_bar.as_mut().unwrap().get_ui_component_mut() };
...
impl ShieldPointWidget {
    pub fn create_shield_point_widget(parent_widget: &mut dyn Widget) -> ShieldPointWidget {
        ...
        let shield_point_bar = create_hit_point_bar_widget(unsafe { &mut *shield_point_layer }, get_color32(75, 75, 255, 75));
    ...
    pub fn update_shield_point_widget(&self, shield_point: f32, max_shield_point: f32) {
        ...
        let shield_point_ui = unsafe { self._shield_point_layer.as_mut().unwrap().get_ui_component_mut() };
        shield_point_ui.set_text(&format!("Shield: {}", shield_point as i32));
        let shield_point_bar = unsafe { self._shield_point_bar.as_mut().unwrap().get_ui_component_mut() };
