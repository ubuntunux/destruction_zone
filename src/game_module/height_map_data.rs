...
impl HeightMapData {
    ...
    pub fn get_height(&self, pos: &Vector3<f32>) -> f32 {
        let pixel_pos_x = (pos.x - &self._bounding_box._min.x) / self._bounding_box._size.x;
        let pixel_pos_y = (pos.z - &self._bounding_box._min.z) / self._bounding_box._size.z;
        let pixel_pos_x = pixel_pos_x.min((self._width - 1) as f32).max(0.0);
        let pixel_pos_y = pixel_pos_y.min((self._height - 1) as f32).max(0.0);
        ...
        lerp(height_data_0, height_data_1, pixel_pos_y_frac) as f32 * self._bounding_box._size.y
    }
}